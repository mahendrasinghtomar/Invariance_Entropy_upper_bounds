...
*
* Float in place of decimal. Will it resolve the error with dtControl(with float) for logreg+maxfreq?
* => Yes, for the hyperbolic example.
* 
*with DTCONTROL = 0:
...
float sup(interval& I){
    return I.upper();
}

float inf(interval& I){
    return I.lower();
}

float mag(interval& I){
    return boost::numeric::norm(I);
}

float midpoint(interval& I){
    return boost::numeric::median(I);
}

float radius(interval& I){
    return 0.5 * boost::numeric::width(I);
}

#define SCOTS_FOCUS_ONLY_INTERSECTION 0 //if 0 then SCOTS-sbstraction bypassed during longer length control sequence
...
float epsil = 0.08;
float eta_s = 0.009;     //std::pow(10,-2);
float x_rad = 5.9574/2.0;  // 2.9787
float eta_i = 0.01;
 ...
 */
using state_type = std::array<float,state_dim>;
//using input_type = std::array<float,input_dim>;
...
#if (HYPERBOLIC_STEP==1) 
    {
        ...
        /* for fwd system */
        {
            ...
            if (!getrusage(RUSAGE_SELF, &usage))
                std::cout << "Memory per transition: " << usage.ru_maxrss / (float)tf_fwd.get_no_transitions() << "\n" << "Memory = " << usage.ru_maxrss << "\n";
...
#if NOTVISUALSTUDIO
            if (!getrusage(RUSAGE_SELF, &usage))
                std::cout << "Memory per transition: " << usage.ru_maxrss / (float)tf.get_no_transitions() << "\n" << "Memory = " << usage.ru_maxrss << "\n";
#endif
            tt.toc();
        ...
        wfile(domain_intersection, "domain_intersection", 1, 0, Example);	
        //state_type dummy1;
        wfile_pFacesSafeset(ss , eta, domain_intersection);
        if(domain_intersection.size()==0) return 1;
...
//// begin 09 March 2021
    ...
    then compare the size of the domain with the size of the winning domain of the computed controller.*/
    {
        ...
        std::cout << "\nThe domain is controlled invariant\n";
    }
//// end 09 March 2021
#endif // 1/0
